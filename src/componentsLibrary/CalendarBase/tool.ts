import { YMD, CalculusMouth, ListType } from './types'
class CalendarTool {
  // 获取这个月多少天
  ...
  // 计算一个月的日周关系
  private static calculationDayMonth({ oneWeek, months }: CalculusMouth): ListType[] {
    const list = []
  ...
  calculusMouth({ oneWeek, months, oldMonths }: CalculusMouth): ListType[] {
    // 得到天数和周几对应关系
    let list: ListType[] = []
    list = list.concat(CalendarTool.calculationDayMonth({ oneWeek, months }))
    // 得到1号之前的日周关系
    ...
    list = list.concat(CalendarTool.calculationDayMonth({ oneWeek: nextOneWeek, months: 28 }))
